...
echo Windows PowerShell is needed but seems to be replaced with PowerShell Core. Aborting...
echo %cBlueHL%Check this webpage for help - %cReset% %cYellow%https://cs.rin.ru/forum/viewtopic.php?f=14^&t=156435%cReset%
echo.
echo %cYellow%Press any key to exit...%cReset%
pause >nul
...
echo  %cRedHL%FACEIT AC may prevent the driver from starting. In rare cases, Vanguard may trigger a%cReset%
echo  %cRedHL%BSOD after disabling driver signature enforcement.%cReset%
echo.
...
if defined wmifailed (
    ...
    echo.
    echo %cYellow%Press any key to return to the main menu...%cReset%
    pause >nul
    goto :dk_prompt
)
...
set "vtx=0"
set "hvpresent=0"
for /f "delims=" %%s in ('%psc% "(gcim Win32_ComputerSystem).HypervisorPresent"') do (
...
if exist "C:\Program Files\FACEIT AC\" (
    echo.
    echo %cRedHL%FACEIT Anti-Cheat has been detected.%cReset%
    echo.
    echo %cRedHL%It is known to block the driver from loading.%cReset%
    echo %cRedHL%Uninstall it before proceeding.%cReset%
    echo.
    echo %cYellow%Press any key to exit...%cReset%
    pause >nul
    exit /b
...
echo(________________________________________________________________________
if not "!dse!"=="1" bcdedit /set {current} onetimeadvancedoptions on >nul
echo.
...
echo(________________________________________________________________________
if not "!dse!"=="1" (
    reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\RunOnce" /v "VBSAdvancedOptions" /t REG_SZ /d "bcdedit /set {current} onetimeadvancedoptions on" /f >nul
)
...
echo %cRed%[Not Responding]%cReset%
...
%psc% "& {(Get-ItemProperty -path HKLM:\Software\Microsoft\wbem\Cimom).'Autorecover MOFs'}" >"%temp%\moflist.txt"
type nul>"%temp%\mofcomp.txt"
for /f "usebackq delims=" %%G in ("%temp%\moflist.txt") do (
    find "%%G" "%temp%\mofcomp.txt" 1>nul || echo %%G>>"%temp%\mofcomp.txt"
)
for /f "usebackq delims=" %%G in ("%temp%\mofcomp.txt") do mofcomp "%%G" >nul 2>&1
